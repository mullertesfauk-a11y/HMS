...
export function AdminShell({
  ...
  return (
    <div className="flex min-h-screen bg-background">
      {/* Sidebar */}
      <aside className="fixed inset-y-0 left-0 z-20 flex w-60 flex-col border-r border-sidebar-border bg-sidebar shadow-xl">
        {/* Brand Header */}
                      ...
                      <li key={item.href}>
                        {item.soon ? (
                          <span
                            ...
                            aria-disabled
                            className="flex items-center gap-3 rounded-md px-3 py-2 text-xs font-medium text-white/50"
                          >
                            <Icon aria-hidden className="h-4 w-4 opacity-50" />
                            <span className="flex-1">{item.label}</span>
                            <span className="rounded bg-black/20 px-1.5 py-0.5 text-[9px] font-medium text-white/70">
                              Soon
                            </span>
                          </span>
                        ) : (
                          <Link
                            ...
                            className={cn(
                              ...
                              active
                                ? "bg-sidebar-active text-white shadow-xs font-semibold backdrop-blur-xs border border-white/20"
        ...
        {/* User Card & Sign Out */}
        <div className="border-t border-sidebar-border p-3 space-y-2 bg-black/15">
          <div className="flex items-center gap-3 px-2 py-1.5">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-white/20 font-serif text-xs font-bold text-white border border-white/20">
              {initials}
            </div>
          ...
          <button
            ...
            disabled={signingOut}
            className="flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-xs font-medium text-white/75 transition-colors hover:bg-white/10 hover:text-white disabled:opacity-50"
          >
      ...
      <div className="ml-60 flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-border bg-white/95 backdrop-blur-xs px-6 shadow-2xs">
          <div className="flex items-center gap-2.5">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-2.5 py-0.5 text-[11px] font-semibold text-[#004fff] border border-blue-200/70">
              <span className="h-1.5 w-1.5 rounded-full bg-[#004fff] animate-pulse" />
              Live Operations
            </span>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-blue-50 border border-blue-200/80 text-xs font-bold text-[#004fff]">
                {initials}
              </div>
            </div>
          </div>
